...
SampleAnnotationFile <- 
    dir(file.path(datadir, "annotation"), pattern = "final.xlsx$",
        full.names = TRUE, recursive = TRUE)
...
```{r}
dataEV_D14_original <- dataEV[, pData(dataEV)$Days == "14-Day"]

dataEV_D01_original <- dataEV[, pData(dataEV)$Days == "1-Day"]
```

```{r}
# 
# summary_table <- pData(dataEV_D14_original) %>%
...
# 
# write_xlsx(x = summary_table, path = paste0(getwd(), "/output_original.xlsx"))
```
...
count_mat <- dplyr::count(pData(dataEV), Tissue, Treatment, Days, Segment)
...
Sankey_All <- plot_sankey(count_mat = dplyr::count(pData(dataEV), Tissue, Treatment, Days, Segment))


Sankey_D14 <- plot_sankey(count_mat = dplyr::count(pData(dataEV_D14_original), Tissue, Treatment, Segment))


Sankey_D1 <- plot_sankey(count_mat = dplyr::count(pData(dataEV_D01_original), Tissue, Treatment, Segment))

Sankey_All | (Sankey_D1 / Sankey_D14)


get_counts <- function(data = dataEV_D01, treatment = "PBS", segment = "Ciliated"){
  
  data <- dplyr::count(pData(dataEV_D01), Tissue, Treatment, Segment)
  
  subset <- data[data$Treatment == treatment & data$Segment == segment, ]
...
# select the annotations we want to show, use `` to surround column names with
# spaces or special symbols
count_mat <- dplyr::count(pData(target_dataEV), Patient, Treatment, Days, Segment)
...
write_xlsx(x = summary_table, path = paste0(getwd(), "/output_postnorm.xlsx"))
```
...
```{r Supplemental Fig 2, fig.height=11, fig.width=20}
