...
EV_protein_OVCAR3 <- read_xlsx(path = paste0(source_path, "/differential_protein_expression_EV_proteins.xlsx"), col_types = c("text", "numeric", "numeric", "numeric", "numeric"))


EV_protein_FT240 <- read_xlsx(path = paste0(source_path, "/differential_protein_expression_EV_proteins.xlsx"), col_types = c("text", "numeric", "numeric", "numeric", "numeric"), sheet = "PBS vs FT240")


EV_protein_OVCAR3 <- EV_protein_OVCAR3 |> dplyr::rename(c(Marker.name = gene_symbol, log_fold = logFC, p_value_t = P.Value, minus_log10.p.value_t = `Log 10 p value`))

EV_protein_FT240 <- EV_protein_FT240 |> dplyr::rename(c(Marker.name = gene_symbol, log_fold = logFC, p_value_t = P.Value, minus_log10.p.value_t = `Log 10 p value`))
...
VP_OVCAR3 <- volcano_plot(df = EV_protein_OVCAR3, xmin = -6, xmax = 6, num_genes = 0, title_name = "hFTE EV proteome change following EV insult", upreg = "OVCAR3", downreg = "PBS") |> add_labels(df = EV_protein_OVCAR3, n_genes = 40, lab_size = 6, x_lim_max = 6)

VP_FT240 <- volcano_plot(df = EV_protein_FT240, xmin = -6, xmax = 6, num_genes = 0, title_name = "hFTE EV proteome change following EV insult", upreg = "FT240", downreg = "PBS") |> add_labels(df = EV_protein_FT240, n_genes = 40, lab_size = 6, x_lim_max = 6)
