...
to export the number of segments for supplemental table 2

```{r}

# summary_table <- pData(dataEV_D01) %>%
#   filter(!is.na(Segment), !is.na(Treatment)) %>%  # filter out blanks
#   mutate(group = paste(Treatment, Segment, sep = ", ")) %>%
#   mutate(group =
#             factor(group, 
#                    levels = c("PBS, Ciliated", "PBS, Secretory","FT240, Ciliated", "FT240, Secretory", "OVCAR3, Ciliated", "OVCAR3, Secretory")
#                    )
#           ) |>
#     mutate(patient =
#             factor(Patient, 
#                    levels =  c("pt1", "pt2", "pt3", "pt4", "pt5", 
#                                "pt6", "pt7", "pt8", "pt9", "pt10", 
#                                "pt11", "pt12", "pt13", "pt14", 
#                                "pt15", "pt16", "pt17")
#                    )
#             ) |>
#   count(Patient, group) %>% arrange(Patient) |>
#   pivot_wider(names_from = group, values_from = n, values_fill = 0)
# 
# write_xlsx(x = summary_table, path = paste0(getwd(), "/output.xlsx"))
...
source <- paste0(getwd(), "/GO_analysis/")
...
highlight_dow_OVCAR3_sec <-  sec_D1_PBS.vs.OVCAR3[sec_D1_PBS.vs.OVCAR3$log_fold < -0.5 & sec_D1_PBS.vs.OVCAR3$p_value_t < 0.05, ]$Marker.name
...
create_heatmap <- function(dataset = EV_1_day_sec, 
  ...
  averaged_df <- merged_df %>%
    ...
    dplyr::summarise(across(where(is.numeric), \(x) mean(x, na.rm = TRUE)))  # Average each row, excluding non-numeric columns
...
# write_xlsx(cil_v_sec_t.test_D01, path = paste0(getwd(), "/1Day_CILIATED.vs.SECRETORY_segment.comparison_ttest", ".xlsx"))
...
# write_xlsx(cil_v_sec_t.test_D14, path = paste0(getwd(), "/14Day_CILIATED.vs.SECRETORY_segment.comparison_ttest", ".xlsx"))
...
```{r}
kegg_and_reactome_files <- paste0(getwd(), "/GO_analysis/kegg_and_reactome/files_for_plotting")
